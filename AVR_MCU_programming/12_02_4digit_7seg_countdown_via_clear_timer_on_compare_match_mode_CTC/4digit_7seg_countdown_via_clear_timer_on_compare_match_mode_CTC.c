...
int COUNTER_MIN_VAL = 31;
int seconds_counter = 38;
...
ISR(TIMER1_COMPA_vect) {
	if (seconds_counter > COUNTER_MIN_VAL) {
		--seconds_counter;
	}
...
void ConfigureStandaloneLED() {
	DDRC |= 1<<0 | 1<<0;      // set PC0 and PC1 as OUTPUT
	PORTC &= ~(1<<0 | 1<<1);  // set PC0 and PC1 as LOW level
}

void ConfigureTimerCounters() {
