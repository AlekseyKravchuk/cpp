...
class ReserveProxyObj {
   private:
    size_t _proxyCapacity;

   public:
    ReserveProxyObj(size_t newCapacity) : _proxyCapacity(newCapacity) {}

    size_t GetCapacity() {
        return _proxyCapacity;
    }
};
...
class SimpleVector {
   public:
    ...
    // конструктор,позволяющий устанавливать _capasity при нулевом размере
    // также принимает тип "size_t" (но в завуалированном виде) через "ReserveProxyObj"
    SimpleVector(ReserveProxyObj proxyObj) : _arrayPtr(proxyObj.GetCapacity()),
                                             _size(0),
                                             _capacity(proxyObj.GetCapacity()) {}
    ...
    void Reserve(size_t new_capacity) {
        if (new_capacity > _capacity) {
            // перевыделяем память
            auto tmpVec = SimpleVector<Type>(ReserveProxyObj(new_capacity));

            // и копируем элементы
            std::copy(begin(), end(), tmpVec.begin());

            // восстанавливаем корректное значение "_size"
            tmpVec._size = _size;

            // меняем содержимое текущего вектора и временного
            swap(tmpVec);
            // после завершения метода "SimpleVector::Reserve()", временный массив будет уничтожен
        }
    }
