...
struct Document {
    ...
    /* explicit */ Document() : id(0), relevance(0.0), rating(0) {}
...
class SearchServer {
   public:
    explicit SearchServer(const string& spaceSeparatedStopWords) { SetStopWords(spaceSeparatedStopWords); }
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query,
        ...
        for (const string& word : query.plus_words) {
            ...
            if (_word_docID_freqs.at(word).count(document_id)) {
   ...
   private:
    ...
    vector<Document> FindAllDocuments(const Query& query, Predicate pred) const {
        ...
        for (const string& word : query.plus_words) {
            ...
            for (const auto [docID, term_freq] : _word_docID_freqs.at(word)) {
