...
class SimpleVector {
   ...
   public:
    ...
    SimpleVector& operator=(const SimpleVector& rhs) {
        // 1) Проверяем, не выполняется ли присваивание массива самому себе (громоздкости от проверки не добавилось)
        assert(arr_.Get() != rhs.arr_.Get());

        // 2) создаем временный массив такой же емкости, как и "rhs"
        SimpleVector<Type> tmp(Reserve(rhs.GetCapacity()));

        // 3) копируем в него элементы из rhs
        std::copy(rhs.begin(), rhs.end(), tmp.begin());

        // 4) Обмениваем содержимое временной копии и текущего объекта.
