...
class SimpleVector {
   public:
    ...
    SimpleVector(ReserveProxyObj reserved)
        : items_(reserved.capacity), size_(0), capacity_(reserved.capacity) {
    }
    ...
    void Reserve(size_t new_capacity) {
        if (new_capacity > capacity_) {
            auto new_items = ReallocateMove(new_capacity);  // может бросить исключение
    ...
    void Resize(size_t new_size) {
        if (new_size > capacity_) {
            ...
            // Перемещаем существующие элементы вектора на новое местое
            auto new_items = ReallocateMove(new_capacity);  // может бросить исключение
   ...
   private:
    // Перемещает элементы массива на новое место и возвращает указатель на него
...
inline bool operator==(const SimpleVector<Type>& lhs, const SimpleVector<Type>& rhs) {
    return (lhs.GetSize() == rhs.GetSize()) && std::equal(lhs.begin(), lhs.end(), rhs.begin());  // может бросить исключение
}
