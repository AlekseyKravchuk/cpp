...
size_t GetSynonymsCount(const Synonyms& synonyms,
    ...
    return synonyms.at(word).size();
}

bool AreSynonyms(const Synonyms& synonyms,
                 const std::string& first_word,
                 const std::string& second_word) {
    return synonyms.at(first_word).count(second_word);
}
...
void TestAreSynonyms() {
    {
        const Synonyms emtpy;
        assert(AreSynonyms(emtpy, "a", "b") != 0);
        assert(!AreSynonyms(emtpy, "b", "a") != 0);
    }
