   ...
   // шаблонный метод класса
    ...
    void RunTest(TestFunc func, const std::string& test_name) {
        try {
            ...
            // когда срабатывает Assert или AssertCount в каком-либо из наших тестов,
    ...
    // В деструкторе проверяем количество упавших тестов. Если упал хотя бы 1 тест, завершаем программу.
    ~TestRunner() {
        if (_fail_count > 0) {
            std::cerr << _fail_count << " tests failed. Terminate." << std::endl;

            // завершаем нашу программу с ненулевым кодом возврата
            std::exit(1);
        }

        // Примечание: если все тесты успешно выполняются, переходим к основному решению
    }
...
// Оборачиваем запуск тестов в отдельную функцию посредством вызова метода TestRunner::RunTest(test_func, name_of_test_func)
// Благодаря инстанцированию класса "TestRunner" в отдельной функции, по закрывающей скобке (по завершению функции)
// вызывается деструктор класса "TestRunner", в котором принимается решение о продолжении выполнения программы
void TestAll() {
    // инстанцируем объект класса "TestRunner" в отдельной функции
    TestRunner test_runner;
    std::cerr << "======================= TESTS ======================="s << std::endl;
    test_runner.RunTest(TestAddSynonyms, "TestAddSynonyms"s);
    test_runner.RunTest(TestAreSynonyms, "TestAreSynonyms"s);
    test_runner.RunTest(TestGetSynonymsCount, "TestGetSynonymsCount"s);
    std::cerr << "===================== END TESTS ====================="s << std::endl;
} // по этой закрывающей скобке выполняется ДЕСТРУКТОР класса TestRunner

int main() {
#ifdef _GLIBCXX_DEBUG
    // === debug mode is enabled, input redirection from stdin to file ===
