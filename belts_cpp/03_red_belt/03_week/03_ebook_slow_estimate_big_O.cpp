// Оцените сложность опубликованного нами медленного решения задачи «Электронная книга»:
...
class ReadingManager {
   public:
    ...
    void Read(int user_id, int page_count) {
        ...
        while (
            position > 0 && page_count > user_page_counts_[sorted_users_[position - 1]]) {
    ...
    double Cheer(int user_id) const {
        ...
        while (
            position < user_count && user_page_counts_[sorted_users_[position]] == page_count) {
            ++position;
        }
