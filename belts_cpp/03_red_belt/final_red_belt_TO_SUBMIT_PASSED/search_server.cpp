...
void SearchServer::UpdateDocumentBase(std::istream &document_input) {
    ...
    for (std::string current_document;
         std::getline(document_input, current_document);) {
...
void SearchServer::AddQueriesStream(std::istream &query_input,
    ...
    for (std::string current_query; getline(query_input, current_query);) {
        ...
        std::partial_sort(
            ...
            std::begin(search_result) +
                std::min<size_t>(ANSWERS_COUNT, search_result.size()),
...
//----------------------------------------------------------------------------------------------------
std::vector<std::pair<size_t, size_t>> InvertedIndex::Lookup(
