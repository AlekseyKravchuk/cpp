...
class TestRunner {
    ...
    void RunTest(TestFunc func, const std::string& test_name) {
        try {
            ...
            std::cerr << _times_runtest_called << ") "
                      << std::setfill('-') << std::setw(40) << std::left
   ...
   private:
    ...
    size_t _times_runtest_called = 0;  // счетчик количества запусков метода "RunTest" (для указания порядковых номеров вызовов тестов)
};
