...
struct Account {
    int balance = 0;            // <== это поле является SHARED DATA, т.л. к нему одновременно обращаются несколько потоков
    ...
    bool Spend(int value) { // "value" - это сумма, которую мы хотим потратить с нашего счета
        ...
        if (value <= balance) {
            balance -= value;
            history.push_back(value); // благодаря тому, что защитили крит.секцию мьютеском, всё ОК, НЕТ "malloc(): corrupted top size"
