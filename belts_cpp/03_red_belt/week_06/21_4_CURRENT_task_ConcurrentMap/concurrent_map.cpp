...
class ConcurrentMap {
  public:
    ...
    Access GetAccessToSubdict(K index) {
        return {std::lock_guard<std::mutex>(_guards[index]), _subdict_store[index]};
    }
    ...
    Access operator[](const K& key) {
        ...
        // return Access{std::lock_guard<std::mutex>(_guards[i]), _subdict_store[i][key]};

        if (_subdict_store[i].count(key)) {
            return Access{std::lock_guard<std::mutex>(_guards[i]), _subdict_store[i].at(key)};
        } else {
            std::lock_guard<std::mutex> guard(_guards[i]);
            V& ref_value = _subdict_store[i].insert({key, V()}).first->second;

            return {std::lock_guard<std::mutex>(_guards[i]), ref_value};
        }
    }
