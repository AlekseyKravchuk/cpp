...
void Test_Duration() {
    const std::vector<std::string> docs = {
        ...
        "using auto promotes good object-oriented practices such as preferring interfaces over implementations the lesser the number of types specified the more generic the code is and more open to future changes which is a fundamental principle of object-oriented programming the initialization expression which could probably mean searching through the code for function return types",
        ...
        "the auto can be used to specify the return type from a function in c++11 this requires a trailing return type in the function declaration in c++14 this has been relaxed and the type of the return value is deduced by the compiler from the return expression if there are multiple return values they should have the same type concern for actual types that we don't care about anyways  it is very often that even though we explicitly specify the type  we don't actually care about it  a very common case is with iterators  but one can think of many more  when you want to iterate over a range  you don't care about the actual type of the iterator  you are only interested in the iterator itself; so  using auto saves time used for typing possibly long names and helps you focus on actual code and not type names  in the following example  in the first for loop  we explicitly use the type of the iterator  it is a lot of text to type  the long statements can actually make the code less readable  and you also need to know the type name that you actually don't care about  the second loop with the auto specifier looks simpler and saves you from typing and caring about actual types",
        ...
        "b4 is initialized with a char since an implicit conversion to size_t exists the  string functions mostly assume zero termination and thus can buffer overflow constructor with a single parameter will be called the intention here is not necessarily clear perhaps it should have been instead of 'a' in which case the third constructor would have been called however b5 is most likely an error because maxsize is an enumerator representing an itemsizes and should have nothing to do with a string buffer size these erroneous situations are not flagged by the compiler in any way using the explicit specifier in the declaration of a constructor that constructor becomes an explicit constructor and no longer allows implicit constructions of objects of a class type to exemplify this we will constructor becomes an explicit constructor and no longer allows implicit constructions of objects of a class type to exemplify slightly change the string_buffer class earlier to declare all constructors explicit"};
