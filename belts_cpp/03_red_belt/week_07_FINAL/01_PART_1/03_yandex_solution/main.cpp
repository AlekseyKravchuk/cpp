...
std::string GenerateWord(int SIZE) {
    return std::string(SIZE, 'a');
}

std::vector<std::string> GenerateSentence(int SIZE = 50000) {
    ...
    for (int i = 0; i < SIZE; ++i) {
        result.push_back(GenerateWord(1));
    }

    return result;
}

auto docs = GenerateSentence();
auto queries = GenerateSentence(500'000);
auto expected = GenerateSentence();

void TestSentenceTime() {
    TestFunctionality(docs, queries, expected);
}

int main() {
    TestRunner tr;
    RUN_TEST(tr, TestSentenceTime);
