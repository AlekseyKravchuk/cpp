...
struct Hasher {
    ...
    size_t operator()(const Point3D& p) const {
        ...
        // свободный член можем положить равным нулю, т.к. он не влияет на коллизии, а лишь циклически смещает бакеты
        const size_t x = 8'539;
        // const size_t x = 2'946'901;

        return int_hasher(p.x) * x * x + int_hasher(p.y) * x + int_hasher(p.z);
    }
