...
struct Address {
    ...
    bool operator==(const Address& other) const {
        return std::tuple(city, street, building) ==
               std::tuple(other.city, other.street, other.building);
    }
};
...
struct Person {
    ...
    bool operator==(const Person& other) const {
        return std::tuple(name, height, weight, address) ==
               std::tuple(other.name, other.height, other.weight, other.address);
    }
};

struct AddressHasher {
    size_t operator()(const Address& addr) const {
        size_t h_city = str_hasher(addr.city);
        size_t h_street = str_hasher(addr.street);
        size_t h_building = int_hasher(addr.building);

        const int x = 8'783;
        return h_city*x*x + h_street*x + h_building;
    }
...
struct PersonHasher {
    size_t operator()(const Person& person) const {
        size_t h_name = str_hasher(person.name);
        size_t h_height = int_hasher(person.height);
        size_t h_weight = double_hasher(person.weight);
        size_t h_address = address_hasher(person.address);

        const int x = 33'713;
        return h_name*x*x*x + h_height*x*x + h_weight*x + h_address;
    }
