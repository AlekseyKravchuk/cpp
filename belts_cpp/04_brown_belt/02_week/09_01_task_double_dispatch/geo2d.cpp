...
bool Collide(Circle c, Segment s) {
  if (
    ...
    // Высота треугольника (s.p1, s.p2, c.center), проведённая из c.center, попадает на отрезок (s.p1, s.p2).
    ...
    // числах, возведём сравниваемые величины в квадрат и сравним (2S)^ 2 с R^2 * |s.p1, s.p2|^2
