...
class CommentServer {
    ...
    HttpResponse ServeRequest(const HttpRequest& req) {
        ...
        if (req.method == "POST") {
            if (req.path == "/add_user") {
                auto userId = _users.size();
                _users[userId];
                response.SetCode(HttpCode::Ok).SetContent(to_string(userId));
            } else if (req.path == "/add_comment") {
                auto [userId, comment] = ParseIdAndContent(req.body);
                if (_users.at(userId)._isBanned) {
                    _lastComment = {userId, 4};
                    response.SetCode(HttpCode::Found).AddHeader("Location", "/captcha");
                } else if (_lastComment && _lastComment->user_id == userId) {
                    ++_lastComment->consecutive_count;
                    if (_lastComment->consecutive_count > SPAM_LIMIT) {
                        // user is spammer
                        _users[userId]._isBanned = true;
                        response.SetCode(HttpCode::Found).AddHeader("Location", "/captcha");
                    } else {
                        _users[userId]._comments.push_back(move(comment));
                        response.SetCode(HttpCode::Ok);
                    }
                } else {
                    _lastComment = {userId, 1};
                    _users[userId]._comments.push_back(move(comment));
                    response.SetCode(HttpCode::Ok);
                }
                ...
                if (answer == "42") {
                    _users.at(userId)._isBanned = false;

                    if (_lastComment && _lastComment->user_id == userId) {
                        _lastComment->consecutive_count = 0;
                    }
            ...
            if (req.path == "/user_comments") {
                ...
                string content;
                for (const auto& comment : _users.at(userId)._comments) {
                    content += comment + '\n';
                }
  ...
  private:
    ...
    unordered_map<size_t, UserInfo> _users;
    optional<LastCommentInfo> _lastComment;
};
