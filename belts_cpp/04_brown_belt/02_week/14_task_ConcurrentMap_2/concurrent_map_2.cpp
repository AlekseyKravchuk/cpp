...
public:
    ...
    struct WriteAccess {
        lock_guard<mutex> write_guard;
        V& ref_to_value;
    };

    struct ReadAccess {
        lock_guard<mutex> read_guard;
        const V& ref_to_value;
    };

    struct Bucket {
        MapType data;  // unordered_map<K, V, Hash>
        ...
        WriteAccess GetValue(const K& key) { return {lock_guard(bucket_mutex), data[key]}; }

        ReadAccess GetValue(const K& key) const { return {lock_guard(bucket_mutex), data.at(key)}; }
    };
