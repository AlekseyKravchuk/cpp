...
// Данная реализация интерфейса IBook позволяет отследить объём памяти, в данный момент занимаемый всеми загруженными
// книгами. Для тестирования своей программы вы можете написать другую реализацию, которая позволит также убедиться,
// что из кэша выгружаются в первую очередь наименее используемые элементы. Собственно, тестирующая система курсеры
// имеет как раз более продвинутую реализацию.
    ...
    Book(string name,
         ...
         atomic<size_t>& memory_used_by_books)
            : _name(std::move(name)),
              ...
              _memory_used_by_books(memory_used_by_books) {
        _memory_used_by_books += _content.size();
    }
...
private:
  string _name;
  string _content;
  atomic<size_t>& _memory_used_by_books;
};

// Данная реализация интерфейса IBooksUnpacker позволяет отследить объём памяти, в данный момент занимаемый всеми
// загруженными книгами и запросить количество обращений к методу UnpackBook().
// Для тестирования своей программы вы можете написать другую реализацию.
// Собственно, тестирующая система курсеры имеет как раз более продвинутую реализацию.
...
void TestCaching(const Library& lib) {
  ...
  // замещения элементов в кэше. Для этих целей можете написать тест самостоятельно.
