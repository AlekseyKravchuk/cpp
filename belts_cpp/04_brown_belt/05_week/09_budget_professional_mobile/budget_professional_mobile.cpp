...
class BulkLinearUpdater {
  public:
    ...
    BulkLinearUpdater(const BulkMoneySpender &spend) : _spent(spend) { }
    ...
    void CombineWith(const BulkLinearUpdater& other) {
        _add.delta = _add.delta * other._tax.ComputeFactor() + other._add.delta;
        _spent.delta += other._spent.delta;
        _tax.factor *= other._tax.factor;
    }

    pair<double, double> Collapse(double original_income,
                                  double original_spent,
                                  IndexSegment segment) const {
        return {original_income * _tax.ComputeFactor() + _add.delta * segment.length(),
                original_spent + _spent.delta * segment.length()};
    }
...
class SummingSegmentTree {
  public:
    SummingSegmentTree(size_t size) : _root(Build({0, size})) {}

    Data ComputeSum(IndexSegment segment) const {
        return this->TraverseWithQuery(_root, segment, ComputeSumVisitor{});
    }

    void AddBulkOperation(IndexSegment segment, const BulkOperation& operation) {
        this->TraverseWithQuery(_root, segment, AddBulkOperationVisitor{operation});
    }
    ...
    class AddBulkOperationVisitor {
      public:
        ...
        void ProcessFull(const NodeHolder& node) const {
            node->postponed_bulk_operation.CombineWith(_operation);
            tie(node->data, node->spent) = _operation.Collapse(node->data, node->spent, node->segment);
        }
      ...
      private:
        const BulkOperation& _operation;
    };
