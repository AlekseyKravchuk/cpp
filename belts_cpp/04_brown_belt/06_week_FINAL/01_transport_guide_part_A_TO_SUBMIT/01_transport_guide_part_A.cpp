...
class TransportGuide {
  ...
  private:
    ...
    void ParseInputCreationEntry(std::string_view s);
...
void TransportGuide::AddBusRoutes() {
    ...
    for (string_view bus_route_entry : _buffer) {
        ...
        double bus_route_length = std::accumulate(distances.begin(),
                                                  distances.end(),
                                                  0.0);
...
void TransportGuide::ProcessRetrievalQueries(std::istream& in) {
    ...
    for (size_t i = 0; i < retrieve_queries_count; ++i) {
        ...
        if (auto it = _bus_name_to_bus_route.find(bus_route_name_requested);
                it != _bus_name_to_bus_route.end()) {
