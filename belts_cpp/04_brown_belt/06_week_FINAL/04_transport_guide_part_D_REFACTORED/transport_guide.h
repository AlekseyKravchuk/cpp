...
//    const BusRouteStats& GetStatsForBusRoute(std::string_view bus_route_name) const;
  ...
  private:
    std::pair<Stop,
         std::optional<NextStopsToDistancesMap>> GetStopFromJsonObject(const std::map<std::string, Json::Node>& request_as_json_map);
  ...
  private:
    ...
    std::unordered_map<std::pair<CurrentStopName, NextStopName>,
                       DistanceByRoad,
                       PairOfStopNameViewHasher> _stop_names_pair_to_distance;
