...
void TransportManager::InitStop(const string& name) {
  if (!_stop_idx.count(name) || (_stop_idx.count(name) && _stops[_stop_idx[name]].Name() != name)) {
    _stops.emplace_back(name);
      _stop_idx[name] = _stops.size() - 1;
  }
}
...
void TransportManager::AddStop(const string& name, double latitude, double longitude, const unordered_map<string, unsigned int>& distances) {
  ...
  size_t id = _stop_idx[name];
  _stops[id].SetCoordinates(Coordinates{latitude, longitude});

  for (const auto& [stop_name, dist] : distances) {
    InitStop(stop_name);
      _distances[id][_stop_idx[stop_name]] = dist;
    if (!_distances[_stop_idx[stop_name]].count(id) || (_distances[_stop_idx[stop_name]].count(id) && _distances[_stop_idx[stop_name]][id] == 0)) {
        _distances[_stop_idx[stop_name]][id] = dist;
    }
  }
}

void TransportManager::AddBus(const RouteNumber& bus_no, const std::vector<std::string>& stop_names, bool is_roundtrip) {
    _buses[string{bus_no}] = is_roundtrip ? BusRoute::CreateCyclicBusRoute(bus_no, stop_names)
...
std::pair<unsigned int, double> TransportManager::ComputeBusRouteLength(const RouteNumber& route_number) {
  ...
  for (const auto& stop_name : _buses[route_number].Stops()) {
    InitStop(stop_name);
    bus_stops.push_back(&_stops[_stop_idx[stop_name]]);
  }
  ...
  for (size_t i = 0; i < bus_stops.size() - 1; ++i) {
    ...
    distance_road += _distances[_stop_idx[bus_stops[i]->Name()]][_stop_idx[bus_stops[i + 1]->Name()]];
  }
...
void TransportManager::CreateRoutes() {
  ...
  for (const auto& [bus_no, bus_route] : _buses) {
    ...
    for (size_t i = 0; i < bus_stops.size(); ++i) {
      ...
      for (size_t j = i + 1; j < bus_stops.size(); ++j) {
        time_sum += _distances[_stop_idx[bus_stops[j - 1]]][_stop_idx[bus_stops[j]]] / (_routing_settings.bus_velocity * 1000 / 60);
        _road_graph->AddEdge(Graph::Edge<double>{
            .from = 2 * _stop_idx[bus_stops[i]] + 1,
            .to = 2 * _stop_idx[bus_stops[j]],
            .weight = time_sum
        });
  ...
  RouteInfo TransportManager::GetRouteInfo(std::string from, std::string to, size_t request_id) {
    size_t from_id = 2 * _stop_idx[from];
    size_t to_id = 2 * _stop_idx[to];
