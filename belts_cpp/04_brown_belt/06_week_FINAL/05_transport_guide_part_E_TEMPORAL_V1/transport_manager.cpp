...
void TransportManager::InitStop(const string& name) {
    if (!_stop_name_to_idx.count(name) ||
        (_stop_name_to_idx.count(name) && _stops[_stop_name_to_idx[name]].Name() != name)) {
    _stops.emplace_back(name);
      _stop_name_to_idx[name] = _stops.size() - 1;
  }
}

void TransportManager::AddStop(const string& name,
                               double latitude, double longitude,
                               const unordered_map<string,
                               unsigned int>& distances) {
  InitStop(name);

  size_t id = _stop_name_to_idx[name];
  _stops[id].SetCoordinates(Coordinates{latitude, longitude});

  for (const auto& [stop_name, dist] : distances) {
    InitStop(stop_name);
      _distances[id][_stop_name_to_idx[stop_name]] = dist;
    if (!_distances[_stop_name_to_idx[stop_name]].count(id) || (_distances[_stop_name_to_idx[stop_name]].count(id) && _distances[_stop_name_to_idx[stop_name]][id] == 0)) {
        _distances[_stop_name_to_idx[stop_name]][id] = dist;
    }
  }
}

void TransportManager::AddBus(const RouteNumber& bus_no, const std::vector<std::string>& stop_names, bool is_roundtrip) {
    _bus_name_to_bus_route[string{bus_no}] = is_roundtrip ? BusRoute::CreateCyclicBusRoute(bus_no, stop_names)
...
std::pair<unsigned int, double> TransportManager::ComputeBusRouteLength(const RouteNumber& route_number) {
  if (!_bus_name_to_bus_route.count(route_number)) {
    return {0, 0};
  }

  if (auto route_length = _bus_name_to_bus_route[route_number].RouteLength(); route_length.has_value()) {
  ...
  for (const auto& stop_name : _bus_name_to_bus_route[route_number].Stops()) {
    InitStop(stop_name);
  ...
  _bus_name_to_bus_route[route_number].SetRouteLength(distance_road, distance_direct);
...
StopInfo TransportManager::GetStopInfo(const string& stop_name, size_t request_id) {
  ...
  for (const auto& bus : _bus_name_to_bus_route) {
...
BusInfo TransportManager::GetBusInfo(const RouteNumber& bus_no, size_t request_id) {
  if (!_bus_name_to_bus_route.count(bus_no)) {
    return BusInfo{
      .request_id = request_id,
      .error_message = "not found",
    };
  ...
  const auto& bus = _bus_name_to_bus_route.at(bus_no);
  ...
  for (const auto& [bus_name, bus_route] : _bus_name_to_bus_route) {
    ...
    for (size_t i = 0; i < bus_stops.size(); ++i) {
      ...
      for (size_t j = i + 1; j < bus_stops.size(); ++j) {
        time_sum += _distances[_stop_name_to_idx[bus_stops[j - 1]]][_stop_name_to_idx[bus_stops[j]]] / (_routing_settings.bus_velocity * 1000 / 60);
        _road_graph->AddEdge(Graph::Edge<double>{
            .from = 2 * _stop_name_to_idx[bus_stops[i]] + 1,
            .to = 2 * _stop_name_to_idx[bus_stops[j]],
            .weight = time_sum
        });
