...
void TransportManager::AddBus(const RouteNumber& bus_no, const std::vector<std::string>& stop_names, bool cyclic) {
    _buses[string{bus_no}] = cyclic ? BusRoute::CreateCyclicBusRoute(bus_no, stop_names)
...
void TransportManager::CreateRoutes() {
  ...
  for (size_t i = 0; i < _stops.size(); ++i) {
    ...
    _edge_description.push_back(WaitActivity{
      .type = "Wait",
  ...
  for (const auto& [bus_no, bus] : _buses) {
    ...
    for (size_t i = 0; i < bus_stops.size(); ++i) {
      ...
      for (size_t j = i + 1; j < bus_stops.size(); ++j) {
        ...
        _edge_description.push_back(BusActivity{
          .type = "Bus",
          .time = time_sum,
          .bus = bus_no,
