...
class PersonsDB {
  public:
    explicit PersonsDB(istream& input);
    long NumGreaterOrEqualMaturityAge(int maturity_age) const;
    int GetTotalIncome(size_t top_n) const;
  ...
  private:
    ...
    static vector<Person> GetPersonsSortedByAge(istream& input);
...
vector<int> PersonsDB::GetCumulativeIncomes() {
    ...
    sort(cumulative_incomes.begin(), cumulative_incomes.end(), std::greater<>());  // sort in descending order
...
optional<string> PersonsDB::FindPopularName(IteratorRange<Iterator> iter_range) {
    ...
    for (auto it = iter_range.begin(); it != iter_range.end();) {
        ...
        long cur_name_count = std::distance(it, same_name_end);
...
long PersonsDB::NumGreaterOrEqualMaturityAge(int maturity_age) const {
...
int PersonsDB::GetTotalIncome(size_t top_n) const {
    if (top_n < 1) {
        return 0;
    }
