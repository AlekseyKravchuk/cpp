...
namespace JsonArgs {
    ...
    unique_ptr <InCommand> ReadInputCommand(const Node& node) {
        ...
        if (type == "Stop") {
            ...
            double longitude = holds_alternative<int>(longitude_node) ? longitude_node.AsInt()
                                                                      : longitude_node.AsDouble();
            ...
            return make_unique<NewStopCommand>(string{stop_name},
                                               latitude,
                                               longitude,
                                               distances);
        } else if (type == "Bus") {
    ...
    void PrintResults(const std::vector<StopInfo>& stop_info, const std::vector<BusInfo>& bus_info,
                      const std::vector<RouteInfo>& route_data, std::ostream& output) {
        vector<Node> result;
        ...
        for (const auto& route: route_data) {
            ...
            if (route.error_message.has_value()) {
                ...
                for (const auto& item: route.items) {
                    ...
                    items.emplace_back(activity_node);
                }
                route_dict["items"] = items;
                route_dict["total_time"] = route.total_time;
            }

            result.emplace_back(std::move(route_dict));
        }
