...
public:
  using RouteNumber = BusRoute::BusRouteName;

    explicit TransportManager(RoutingSettings routing_settings)
            : _routing_settings(routing_settings) { }

  void AddStop(const std::string& current_stop_name, double latitude, double longitude, const std::unordered_map<std::string, unsigned int>& distances);
  void AddBus(const RouteNumber& route_number, const std::vector<std::string>& stop_names, bool is_roundtrip);
  ...
  void CreateRoutes();
  RouteInfo GetRouteInfo(const std::string& from, const std::string& to, size_t request_id);
private:
