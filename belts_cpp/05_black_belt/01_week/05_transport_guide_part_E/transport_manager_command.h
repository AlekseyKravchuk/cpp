...
struct NewStopCommand : public InCommand {
  public:
    NewStopCommand(std::string name,
                   double latitude,
                   double longitude,
...
struct NewBusCommand : public InCommand {
  public:
    NewBusCommand(std::string name, std::vector<std::string> stops, bool is_cyclic)
            : InCommand(InCommandType::NEW_BUS), _name(move(name)), _stops(move(stops)), _is_roundtrip(is_cyclic) {
    }
    ...
    bool IsRoundtrip() const { return _is_roundtrip; }
...
struct StopDescriptionCommand : public OutCommand {
  public:
    explicit StopDescriptionCommand(std::string name)
            : OutCommand(OutCommandType::STOP_DESCRIPTION), _name(std::move(name)) {}

    StopDescriptionCommand(std::string name, size_t request_id)
            : OutCommand(OutCommandType::STOP_DESCRIPTION), _name(std::move(name)), _request_id(request_id) {}
