...
public:
        ...
        : stop_words_(MakeUniqueNonEmptyStrings(stop_words))
    {
        for (const string& word : stop_words) {
            if (!IsValidWord(word)) {
                throw invalid_argument("Stop words must not contain control characters");
            }
        }
    }
    ...
    void AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
        if (document_id < 0) {
            throw invalid_argument("Document's ID must not be negative");
        }
        if (documents_.count(document_id) == 1) {
            throw invalid_argument("Document's ID must not already exist");
        }
        const vector<string> words = SplitIntoWordsNoStop(document);
        for (const string& word : words) {
            if (!IsValidWord(word)) {
                throw invalid_argument("Document data must not contain control characters");
            }
        }
...
private:
    ...
    QueryWord ParseQueryWord(string text) const {
        ...
        if (IsValidWord(text) && !text.empty() && text[0] != '-') {
            ...
            throw invalid_argument("Invalid word or control character in ParseQueryWord()"s);
        }
    }
...
// по сути страница с набором документов на ней
class IteratorRange {
public:
...
public:
    Paginator(Iterator range_begin, Iterator range_end, size_t page_size) {
        for (Iterator iterator = range_begin; iterator != range_end; ) {
            const size_t current_page_size = distance(iterator, range_end);
            if (current_page_size >= page_size) {
                _pages.push_back(IteratorRange(iterator, next(iterator, page_size)));
                advance(iterator, page_size);
            }
            else {
                _pages.push_back(IteratorRange(iterator, next(iterator, current_page_size)));
                advance(iterator, current_page_size);
            }
        }
    }
...
ostream& operator<<(ostream& os, const IteratorRange& page) {
    for (const auto& iterator = page.begin(); iterator != page.end(); advance(iterator, 1)) {
        os << *iterator;
    }
    os << endl;
    return os;
