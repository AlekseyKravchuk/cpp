...
class SingleLinkedList {
   ...
   public:
    ...
    Iterator InsertAfter(ConstIterator pos, const Type& value) {
        ...
        // Если оператор new выбросит исключение, указатель pos.node_->next_node останется прежним
        Node* insertedNodePtr = new Node(value, pos.node_->next_node);

        pos.node_->next_node = insertedNodePtr;
    ...
    void PopFront() noexcept {
        assert(!IsEmpty());
        // указатель на первый элемент списка, который нужно удалить
        Node* nodeToBeDeletedPtr = head_.next_node;

        // указатель на 2-ой элемент списка, который станет 1-м после PopFront
        head_.next_node = nodeToBeDeletedPtr->next_node;

        // удаляем указатель и освобождаем память в куче
    ...
    Iterator EraseAfter(ConstIterator pos) noexcept {
        ...
        Node* const nodeToErasePtr = pos.node_->next_node;

        // получаем адрес следующего узла
        Node* nodeAfterErasedPtr = nodeToErasePtr->next_node;

        // перенастраиваем указатели
        pos.node_->next_node = nodeAfterErasedPtr;

        // удаляем, освобождаем память в heap'е
        delete nodeToErasePtr;

        // уменьшаем размер
        --size_;

        return Iterator{nodeAfterErasedPtr};
    }
    ...
    void Clear() noexcept {
        ...
        while (head_.next_node != nullptr) {
            // сохраняем head_.next_node во временном указателе
