...
bool sortDocumentsByRelevance(const Ranked& doc1,
                              const Ranked& doc2) {
    // if documents have the same relevance
    if (doc1.relevance == doc2.relevance) {
        return doc1.document_id > doc2.document_id;
    } else {
...
vector<Ranked> FindTopDocuments(const vector<DocumentContent>& documents_db,
                                const set<string>& stop_words,
                                const string& raw_query) {
    vector<Ranked> ranked_docs = FindAllDocuments(documents_db, stop_words, raw_query);
    ...
    if (static_cast<int>(ranked_docs.size()) >= MAX_RESULT_DOCUMENT_COUNT) {
        return vector<Ranked>{ranked_docs.begin(), ranked_docs.begin() + MAX_RESULT_DOCUMENT_COUNT};
    } else {
        return ranked_docs;
    }
}
